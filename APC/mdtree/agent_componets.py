...
COMPONENT_LAYOUT_PROMPT_TEMPLATE = """请扮演一位经验丰富的演示文稿设计师和信息架构师。
...
4.  **内容区可用宽度**: 9.0 英寸 (10.0 - 2*0.5)
5.  **内容区可用高度**: 5.5 英寸 (7.5 - 1.5-0.5)
6.  **内容区起始坐标**: left=0.5, top=1.5
7.  **内容区结束坐标**: right_edge=9.5, bottom_edge=7.0
8.  **原始 Markdown 内容**:
    ```markdown
    {markdown_content}
    ```
9.  **可用的视觉组件类型及其描述**:
    ...
    * **`triple_card` (三卡片组)**: 并排展示三个带有标题和内容的卡片，中间卡片可能有突出显示。
        ...
        * **视觉提示**: 适合比较三个相关项目、特性、方案或阶段性成果。寻找可以将内容平均分为三个逻辑部分的结构，每个部分都有一个小标题和相应的描述性文本。例如，如果Markdown中有三个连续的、结构类似的小节（例如，由 `###` 或 `####` 级别标题引导，每个标题后有一段描述文本），则可能是此组件的候选。
    ...
    * **`parallel_boxes` (并行双框)**: 并排展示两个文本框，每个框内顶部可以有可选的表情符号。
        ...
        * **视觉提示**: 适用于并列展示两项相关信息，例如对比、原因与结果、问题与解决方案、定义与示例等。寻找可以清晰分为两个语义相关且内容量大致均衡的文本块。例如，由两个相邻的、同级别标题引导的段落。
...
4.  **布局设计与组件尺寸估算**: 为每个选用的组件和 `main_text` 块决定其在幻灯片上的精确位置（`left`, `top`）和尺寸（`width`, `height`）。单位为英寸 (Inches)。
    ...
    * 考虑整体的视觉平衡、层级关系和阅读流程。主要内容流向通常为从上到下、从左到右。
    * 定义清晰的幻灯片边距：所有元素必须严格放置在由“已知信息”部分定义的内容区内。具体而言，元素的有效放置区域为：left 从 0.5 开始，且 (left + width) 应 <= 9.5； top 从 1.5 开始，且 (top + height) 应 <= 7.0。”
...
* 布局约束: 所有元素的 left 必须 >= 0.5，且 (left + width) 必须 <= 9.5。所有元素的 top 必须 >= 1.5，且 (top + height) 必须 <= 7.0。严格遵守这些边界至关重要。
...
class ComponentAgent:
    ...
    def get_layout_plan(self, markdown_content: str) -> Optional[Dict[str, Any]]:
        ...
        agent_prompt = COMPONENT_LAYOUT_PROMPT_TEMPLATE.format(markdown_content=markdown_content)
